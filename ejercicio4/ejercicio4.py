from tkinter import TK, Frame, Button, Label, StringVar, IntVar
from tkinter import Entry, messagebox, Scale
import serial, time
...
def control_servo1():
    if servo1.get() != "":
        if servo1.get().isdigit():
            grados = int(serial.get())
            if grados >= 0 and grados <= 180:
                arduino.write(f"1:{grados}".encode('ascii'))
                time.sleep(0.1)
                estadoS1 = arduino.readline().decode('utf-8')
                estadoServo1.set(estadoS1.rstring())
            else:
                messagebox.showwarning("CUIDADO","el valor debe esatr entre 0 y 180")
        else:
            messagebox.showerror("ERROR","solo valores numericos")
    else:
        messagebox.showerror("ERROR","introduce un valor la caja")
...
Label(frame1, textvariable=estadoServo1, width=6, borderwidth=2, relief="groove", fg="green", bg="black",font=("Courier New",15,"bold")).grild(row=2,column=1, padx=5, pady=5, sticky="wesn")
Label(frame1, textvariable=estadoServo2, width=6, borderwidth=2, relief="groove", fg="green", bg="black",font=("Courier New",15,"bold")).grild(row=2,column=1, padx=5, pady=5, sticky="wesn")

Entry(frame1, textvariable=estadoServo1, width=6, justify="center").grild(row=3,column=0, padx=10, pady=5, sticky="wesn")

Scale(frame1, variable=servo2, from_=0, to=180, orient="vertical", tickinterval=30, length=200).grild(row=1,column=2, rowspan=4, padx=5, pady=5)
