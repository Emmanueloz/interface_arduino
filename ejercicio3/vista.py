...
def mostrar_mensaje(tipo, mensaje, icono):
    if icono == 'warning':
        messagebox.showwarning(tipo, mensaje)
    elif icono == 'error':
        messagebox.showerror(tipo, mensaje)
    elif icono == 'info':
        messagebox.showinfo(tipo, mensaje)
...
controlador = Controlador(estadoServo1, estadoServo2, servo1, servo2, barraEstado)
...
def enviar_servo1():
    resultado = controlador.control_servo1(servo1.get())
    mostrar_mensaje(*resultado)

def enviar_servo2():
    resultado = controlador.control_servo2(servo2.get())
    mostrar_mensaje(*resultado)
